...
app.get('/api/products/:id/details', async (req, res) => {
  try {
    ...
    // Calculate stock history (all transactions, sorted by date)
    ...
    // Build stock history from all transactions
    allTransactions.forEach(t => {
      const date = t.timestamp.toISOString().split('T')[0];
      stockHistoryMap.set(date, {
        date: date,
        stock: t.newStock,
    ...
    stockHistory.push(...Array.from(stockHistoryMap.values()).sort((a, b) => 
